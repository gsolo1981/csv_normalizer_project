...
class CSVNormalizer:
    def __init__(self, data_folder='data'):
        self.data_folder = Path(data_folder)
        # Actualizar las columnas de salida - eliminar C_JURIS, XL_INSTITUCION, C_UNID_EJEC, XL_UNID_EJEC
        # Agregar NombreUnidad
        self.output_columns = [
            'fullname', 'email', 'user', 'application', 'NombreUnidad'
        ]
    ...
    def process_sigaf_internos(self, filepath):
        ...
        normalized_data = []
        for _, row in df.iterrows():
            # Para SIGAF Internos: NombreUnidad es la concatenación de C_UNID_EJEC y XL_UNID_EJEC
            c_unid_ejec = str(row.get('C_UNID_EJEC', '')).strip('"')
            xl_unid_ejec = str(row.get('XL_UNID_EJEC', '')).strip('"')
            
            # Concatenar solo si ambos campos tienen valores válidos
            if c_unid_ejec and xl_unid_ejec and c_unid_ejec != 'nan' and xl_unid_ejec != 'nan':
                nombre_unidad = f"{c_unid_ejec} - {xl_unid_ejec}"
            else:
                nombre_unidad = None
    ...
    def process_mia_consulta(self, filepath):
        ...
        for _, row in df.iterrows():
            ...
            normalized_row = {
                ...
                'user': str(row.get('UserName', '')).strip('"'),
                'application': 'MIA',
                'NombreUnidad': None  # Para MIA es NULO
            }
            normalized_data.append(normalized_row)
    ...
    def process_sigaf_externos(self, filepath):
        ...
        for _, row in df.iterrows():
            normalized_row = {
                ...
                'user': str(row.get('N_CUIT', '')).strip('"'),
                'application': 'SIGAF Externos',
                'NombreUnidad': None  # Para SIGAF Externos es NULO
            }
            normalized_data.append(normalized_row)
    ...
    def process_bac_externos(self, filepath):
        ...
        for _, row in df.iterrows():
            ...
            normalized_row = {
                ...
                'user': str(row.get('usuario', '')),
                'application': 'BAC Externos',
                'NombreUnidad': None  # Para BAC Externos es NULO
            }
            normalized_data.append(normalized_row)
    ...
    def process_bac_internos(self, filepath):
        ...
        for _, row in df.iterrows():
            ...
            # Para BAC Internos: usar el campo NombreUnidad del archivo original
            nombre_unidad = row.get('NombreUnidad')
            if pd.isna(nombre_unidad) or str(nombre_unidad) == 'nan' or str(nombre_unidad) == '':
                nombre_unidad = None
            else:
                nombre_unidad = str(nombre_unidad)
            ...
            normalized_row = {
                ...
                'user': str(row.get('usuario', '')),
                'application': 'BAC Internos',
                'NombreUnidad': nombre_unidad  # Usar el campo NombreUnidad del archivo
            }
            normalized_data.append(normalized_row)
    ...
    def clean_data(self, df):
        ...
        # Limpiar espacios en blanco extra
        string_columns = ['fullname', 'email', 'user', 'application', 'NombreUnidad']
    ...
    def save_to_excel(self, df, output_filename='consolidated_data.xlsx'):
        """Guarda el DataFrame consolidado a Excel"""
        output_path = Path(output_filename)
    ...
    def run(self, output_filename='consolidated_data.xlsx'):
        """Ejecuta todo el proceso de normalización"""
        print("Iniciando proceso de normalización de CSV...")
        print("="*50)
